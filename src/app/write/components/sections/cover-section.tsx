...
export function CoverSection({ delay = 0 }: CoverSectionProps) {
	...
	const handleCoverDrop = async (e: React.DragEvent<HTMLDivElement>) => {
		...
		// 处理从图片列表中拖入的情况。
		// 图片列表拖出来的是 <img src="..."> 片段；这里同时兼容旧的 markdown 写法。
		const payload = e.dataTransfer.getData('text/plain') || e.dataTransfer.getData('text/html') || ''
		const m = /<img[^>]+src=["']([^"']+)["']/i.exec(payload) || /!\[\]\(([^)]+)\)/.exec(payload)
		if (m) {
			const target = m[1]
			let foundItem
